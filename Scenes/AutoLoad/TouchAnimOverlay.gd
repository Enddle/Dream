...
func hold_start(pos = Vector2.ZERO):
	...
	tween.tween_property($hold, "scale", Vector2(.8, .8), .5)
	tween.parallel().tween_property($hold, "modulate", Color(.5, .5, .5, .5), .5)
	tween.tween_property($hold, "scale", Vector2(.95, .95), 1.5)
...
func hold_end():
	...
	tween.parallel().tween_property($hold, "scale", Vector2(.95, .95), .2)


func _ready():
	$hold.scale = Vector2(.95, .95)
